...
export class PDFEngine {
  ...
  static async scanForTOC(
    ...
    console.log(
      "PDFEngine: Starting Semantic AI Analysis. Text size:",
    ...
    const detectedLanguage: "ar" | "en" =
      arabicChars.length > accumulatedText.length * 0.1 ? "ar" : "en";
    console.log(
      `PDFEngine: Detected Language: ${detectedLanguage} (${arabicChars.length} chars)`,
    );
    ...
    try {
      ...
      console.error(
        "PDFEngine: AI Parsing failed, falling back to Regex:",
        err,
      );
    }
  ...
  private static async parseWithAI(
    ...
    fd.append("tool", "curriculum-architect");
    ...
    fd.append(
      ...
      JSON.stringify({
        subject: subject,
        language: locale,
        textPreview: text.substring(0, 12000), // For chapter extraction
        semesterDays: 120, // Default assumption, updated later
      }),
    );
