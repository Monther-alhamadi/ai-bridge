...
export function CurriculumArchitect({ locale }: CurriculumArchitectProps) {
  ...
  const [step, setStep] = useState<"onboarding" | "ingestion" | "planning">(
    "onboarding",
  );

  // State for onboarding wizard
  const [config, setConfig] = useState({
    institutionType: "school", // 'school' daily, 'university' weekly
    startDate: new Date().toISOString().split("T")[0],
    endDate: new Date(new Date().setMonth(new Date().getMonth() + 4))
      .toISOString()
      .split("T")[0],
    frequency: 5, // Lessons per week (3-7 school, 1-3 univ)
    lessonsPerDay: 1, // For school multi-lesson days
  });
  ...
  const handleGenerateSchedule = async () => {
    ...
    toast.loading(
      locale === "ar"
        ? "جاري توزيع المنهج الذكي..."
        : "Calculating adaptive timeline...",
    );
    ...
    const effectiveSchedule =
      weeklySchedule.length > 0 ? weeklySchedule : [1, 2, 3, 4, 5];
    ...
    await db.transaction("rw", db.lessons, async () => {
      ...
      for (let i = 0; i < teachingDays.length; i++) {
        ...
        const unit = sourceUnits[contentIndex] || {
          title: `Lesson ${i + 1}`,
          context: "",
        };
        ...
        const isFallbackContent =
          ...
          sourceUnits[contentIndex]?.context === "Generated Fallback";

        const displayTitle =
          isFallbackContent || i >= sourceUnits.length
            ? locale === "ar"
              ? `الدرس ${i + 1}`
              : `Lesson ${i + 1}` // Revert to standard numbering
            : unit.title;
        ...
        const displayContext = isFallbackContent
          ...
          : unit.context ||
            (locale === "ar"
              ? `سياق الدرس: ${displayTitle}`
              : `Context for ${displayTitle}`);
  ...
  const handleExportCalendar = () => {
    ...
    CalendarUtils.downloadICS(
      `${activeBook?.title || "Schedule"}.ics`,
      icsString,
    );
    toast.success(
      locale === "ar" ? "تم تجهيز ملف التقويم!" : "Calendar file ready!",
    );
  };
  ...
  return (
    <ToolPanel
      title={
        locale === "ar" ? "مهندس المنهج الذكي" : "Smart Curriculum Architect"
      }
      description={
        locale === "ar"
          ...
          <div className="bg-card/40 backdrop-blur-md border rounded-3xl p-6 shadow-xl space-y-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-bold">
                {locale === "ar" ? "إعدادات الفصل الدراسي" : "Semester Config"}
              </h3>
            </div>

            {/* Step 1: Select Book */}
            <div className="space-y-3">
              <Label className="uppercase text-xs font-black text-muted-foreground">
                {locale === "ar" ? "1. اختر المادة" : "1. Select Subject"}
              </Label>
                  ...
                  {books.map((book: Textbook) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (book.semesterStart)
                          setConfig((prev) => ({
                            ...prev,
                            startDate: book.semesterStart!,
                          }));
                        if (book.semesterEnd)
                          setConfig((prev) => ({
                            ...prev,
                            endDate: book.semesterEnd!,
                          }));
                      }}
                      className={cn(
            ...
            <div className="animate-in fade-in zoom-in">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-xl">
                  {locale === "ar" ? "معالج الكتب" : "Book Ingestion"}
                </h3>
                <Button variant="ghost" onClick={() => setStep("planning")}>
            ...
            <div className="space-y-6">
              {!lessons || lessons.length === 0 ? (
                <div className="h-[400px] border-2 border-dashed rounded-3xl flex flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    {locale === "ar"
                      ? 'قم باختيار كتاب واضغط "توزيع المنهج"'
                      : 'Select a book and click "Distribute"'}
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-xl flex items-center gap-2">
                        ...
                        {
                          books?.find((b: Textbook) => b.id === selectedBookId)
                            ?.title
                        }{" "}
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            (window.location.href = `/${locale}/tools/teacher`)
                          }
                        >
                          {locale === "ar"
                            ? "عرض الجدول الأسبوعي"
                  ...
                  <div className="grid gap-3 max-h-[600px] overflow-y-auto pr-2">
                    {lessons.map((lesson: Lesson, idx: number) => (
                      <div
                        key={lesson.id}
                        className="group flex items-center gap-4 p-4 bg-card border rounded-2xl hover:shadow-md transition-all"
                      >
                        ...
                        <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button
                            ...
                            className="text-xs"
                            onClick={() =>
                              (window.location.href = `/${locale}/tools/teacher/lesson-planner?lessonId=${lesson.id}`)
                            }
                          >
