...
export const PromptEngine = {
  ...
  generatePrompt: (data: PromptDataObject): string => {
    // Add CURRICULUM_ARCHITECT case
    if (data.toolId === "curriculum-architect") {
      return SYSTEM_PROMPTS.CURRICULUM_ARCHITECT(
        data.locale,
        data.bookMemory || "",
        data.params.textPreview || "",
        data.params.semesterDays || 0,
      );
    }
