export const SYSTEM_PROMPTS = {
  CURRICULUM_ARCHITECT: (
    ...
    return `
You are an elite Curriculum Architect AI. Extract structured chapters/lessons from the textbook index and preview text.

Input:
- Book Index Summary: ${bookIndex}
- Text Preview (first pages): ${textPreview}
- Semester Length: ${semesterDays} teaching days

Task: 
1. Identify 8-16 logical chapters/lessons from the content.
2. Suggest even distribution across ${semesterDays} days.
3. Estimate grade level if possible.

Strict JSON Output:
{
  "chapters": [
    {
      "title": "${isAr ? "عنوان الفصل" : "Chapter Title"}",
      "estimatedPages": number,
      "context": "Brief description"
    }
  ],
  "summary": "${isAr ? "ملخص المنهج" : "Curriculum Summary"}",
  "grade": "Detected grade level (e.g. 10th)",
  "recommendedPace": "${isAr ? "سرعة التدريس المقترحة" : "Recommended Pace"}"
}

Reply ONLY in ${isAr ? "Arabic" : "English"}.`;
  },
...
${
  model === "5E Model"
    ? isAr
      ? `- تهيئة (Engage), استكشاف (Explore), شرح (Explain), توسيع (Elaborate), تقويم (Evaluate)`
      : `- Engage, Explore, Explain, Elaborate, Evaluate`
    : model === "Gagne 9 Events"
      ? isAr
        ? `- جذب الانتباه، تعريف الأهداف، استرجاع السابق، عرض المحتوى، المساعدة، استثارة الأداء، التغذية الراجعة، تقييم الأداء، التعميم`
        : `- Gain attention, Inform objectives, Stimulate recall, Present content, Provide guidance, Elicit performance, Provide feedback, Assess performance, Enhance retention`
      : model === "UDL"
        ? isAr
          ? `- التمثيل (Representation), التعبير (Action/Expression), المشاركة (Engagement)`
